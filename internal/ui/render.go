...
func RenderSuccess(result setup.Result, kind string) {
	...
	fmt.Printf("location %s\n", valueStyle.Render(formatPath(result.TargetPath)))
	fmt.Printf("package manager %s\n", valueStyle.Render(result.UsedPM))
	fmt.Printf("dashboard key name %s\n", valueStyle.Render(result.APIKeyName))
	fmt.Printf("health %s\n", valueStyle.Render(setup.DefaultHTTPURL))
	fmt.Println()
	fmt.Println("Dashboard API Key")
